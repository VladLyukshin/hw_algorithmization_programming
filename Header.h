...
public:
	...
	string generate()
	{
		...
		{
			cout << "Некорректные параметры: длина запрашиваемого пароля меньше минимальной длины!" << endl;
			return "ERROR!";
		}
		else
		{
			vector <int> ar(count_of_symbols); // используем вектор, чтобы на рандомные места в строке поставить рандомный символ требуемого типа
			...
			for (int i = 0; i < count_of_symbols; i++)
			{
				result += "."; // заполним для начала пароль строкой заданной длины заполненной точками, которые будем заменять на требуемое количество заданных типов
			}
			for (int i = 0; i < list_of_criteria.size(); i++)
			{
				int tm = counter;
