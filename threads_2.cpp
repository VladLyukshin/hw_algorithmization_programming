...
vector <int> read_file(string title) // ôóíêöèÿ äëÿ ñ÷èòûâàíèÿ ôàéëà
{
	int tmp;
	vector <int> arr;
	ifstream fin(title);
	...
	fin.close();
	return arr; // âîçâðàùàåì âåêòîð ÷èñåë èç ôàéëà
}


void thr_func(string name, vector <int>& final_array, const int i, vector <int>& my_arr) // ôóíêöèÿ äëÿ ïîòîêîâ, ÷òîáû îíè ïàðàðåëëüíî ñ÷èòûâàëè êàæäûé ñâîé ôàéë
{
	my_arr = read_file(name);
	if (my_arr.empty())
	{
		flag_read = false; // ôëàã íà ïðîâåðêó ñ÷èòàëñÿ ëè ôàéë
	}
}
...
int my_function(int n, string* filenames)
{
	...
	bool flag = false; // ôëàã äëÿ ïîäñ÷åòà ïîñëåäîâàòåëüíîñòåé ÷èñåë
	int result = 0;
